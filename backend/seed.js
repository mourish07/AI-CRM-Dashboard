...
const slug = (s) => s.toLowerCase().replace(/[^a-zA-Z0-9]/g, "");

const COMPANIES = [
  ["Acme Corp", "amecorp.com"],
  ["Globex", "globex.io"],
  ["Initech", "initech.com"],
...
const run = async () => {
  ...
  // Stage order for leads
  const stageOrder = { New: 0, Qualified: 1, Proposal: 2, Won: 3, Lost: 4 };
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const ageDays = status === "Won" || status === "Lost" ? rand(20, 175) : rand(0, 120);

    leadDocs.push({
      owner,
      name,
      email: emailFor(name, domain),
      phone: `+1 555 05${rand(100, 999)}`,
      company,
      status,
      priority: weighted([
        ["High", 35],
      ...
      notes: pick([
        ...
        "Initial conversation scheduled for next week.",
      ]),
      tags: pickSome(["saas", "enterprise", "sab", "priority"], rand(0, 2)),
  ...
  for (let i = 0; i < 28; i++) {
    ...
    taskDocs.push({
      ...
      description: Math.random() < 0.5 ? pick([
        ...
        "Follow up with the procurement team.",
      ]) : "",
      dueDate,
      status,
      priority: weighted([
